...
class Login
{
    ...
    {
        $sql = "SELECT usuarios.id_usuario, personas.nombre, personas.ap_p, personas.ap_m, usuarios.id_tipo_usuario FROM personas, usuarios, sexos, tipos_usuarios 
 where usuarios.nickname='{$this->usuario}' and usuarios.pass='{$this->pass}' and personas.id_usuario=usuarios.id_usuario and u.id_tipo_usuario=tu.id_tipo_usuario 
 and personas.id_sexo=sexos.id_sexo";
    ...
    public function guardar() {
        ...
        if(isset($datos['id_usuario'])) {
            $sql="INSERT INTO personas(nombre, ap_p, ap_m, edad, id_sexo, id_usuario) VALUES 
('{$this->nombre}','{$this->ap_p}','{$this->ap_m}','{$this->edad}','{$this->id_sexo}','{$datos['id_usuario']}')";
            $this->conexion->QuerySimple($sql);
    ...
    public function getOne(){
        $sql="SELECT * FROM usuarios WHERE nickname='{$this->nickname}' AND password='{$this->contraseña}'";
        $dato=$this->conexion->QueryResultado($sql);
        return $dato;
    }
}